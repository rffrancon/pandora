    ...
    for (const auto &sample: samples) {
        ...
        for (const auto &sample_kmer_path : sample->paths[prg_id]) {
            ...
            if (count == 0) {
                ...
                prg->add_sample_covgs_to_vcf(vcf, kmer_prg, vcf_reference_path, sample->name, sample_id);
            } else {
                ...
                prg->add_sample_covgs_to_vcf(vcf, kmer_prg, vcf_reference_path, path_specific_sample_name, sample_id);
            }
            count++;
        }
        sample_id++;
