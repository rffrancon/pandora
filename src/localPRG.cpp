...
std::vector<Path> LocalPRG::shift(Path path) const {
    ...
    for (const auto &p: extended_paths) {
        ...
        while (!short_paths.empty()) {
            path = short_paths.front();
            auto nodes = nodes_along_path(path);
            short_paths.pop_front();

            if (nodes.back()->pos.get_end() == (--(prg.nodes.end()))->second->pos.get_end()) {
                shifted_paths.push_back(path);
            } else if (nodes.back()->pos.get_end() == path.get_end()) {
                for (uint32_t j = 0; j != nodes.back()->outNodes.size(); ++j) {
                    if (nodes.back()->outNodes[j]->pos.length == 0) {
                        short_paths.push_back(path);
                        short_paths.back().add_end_interval(nodes.back()->outNodes[j]->pos);
                    } else {
                        non_terminus = true;
                    }
                }
...
std::vector<Path> LocalPRG::get_extended_paths(const Path &path) const {
    ...
    while (!short_paths.empty()) {
        auto path = short_paths.front();
        auto path_nodes = nodes_along_path(path);
        short_paths.pop_front();

        // if we can extend within the same localnode, do
        if (path.get_end() < path_nodes.back()->pos.get_end()) {
            path.path.back().length += 1;
            k_paths.push_back(path);
        } else if (path.get_end() != (--(prg.nodes.end()))->second->pos.get_end()) {
    ...
    for (const auto &kmernode_ptr: kmernode_path) {
        ...
        for (const auto &interval : kmernode_ptr->path.path) {
            ...
            for (uint32_t l = start; l < end; ++l) {
                ...
                coverages[k][l] = std::max(coverages[k][l], pnode->kmer_prg.nodes[kmernode_ptr->id]->covg[0] +
...
void LocalPRG::add_consensus_path_to_fastaq(Fastaq &output_fq,
    ...
    std::cout << now() << "Find maxpath for " << pnode->get_name() << std::endl;
    float ppath;
    if (bin)
    ...
    std::vector<uint32_t> covgs = get_covgs_along_localnode_path(pnode, lmp, kmp);
    auto mode_covg = mode(covgs);
    auto mean_covg = mean(covgs);
    std::cout << now() << "Found global coverage " << global_covg << " and path mode " << mode_covg << " and mean "
              << mean_covg << std::endl;
